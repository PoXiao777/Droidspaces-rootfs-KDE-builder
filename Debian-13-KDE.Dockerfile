...
RUN sed -i '/en_US.UTF-8/s/^# //' /etc/locale.gen && \
    ...
    useradd -m -s /bin/bash PoXiao && echo "PoXiao:12345678" | chpasswd 
...
RUN <<'EOF_RUN'
    if [ "$ENABLE_srf_ARG" = "true" ]; then
    mkdir -p /home/PoXiao/.config/autostart
    cat <<'EOF' > /home/PoXiao/.config/autostart/fcitx5.desktop
[Desktop Entry]
Name=Fcitx5
...
EOF
    ...
    echo 'export XDG_RUNTIME_DIR=/run/user/$(id -u)' >> /home/PoXiao/.bashrc
    if [ "$BUILD_KDE" = "min" ] || [ "$BUILD_KDE" = "conc" ] ; then
    mkdir -p /home/PoXiao/.config 
    cat <<'EOF' > /home/PoXiao/.config/kwinrc
[Compositing]
Enabled=false
EOF
    fi
    chown -R PoXiao:PoXiao /home/PoXiao
EOF_RUN
...
usermod -a -G aid_inet,aid_net_raw,input,video,tty,sudo,droidspaces-gpu PoXiao || true
